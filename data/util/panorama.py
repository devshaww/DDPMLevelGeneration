...
is_origin: determine if it needs mask and tranforms, no if set to True
is_left: determine if it needs left uncrop mask or right uncrop mask

return: format like dataset's get_item, but with left or right half or both(when is_origin is True) filled with input
"""

# if is_origin, input[0] is ndarray, (h,w,c)
# if !is_origin, input[0] is tensor ranging between [-1,1], (c,h,w)
# so squeeze is applied to get h and w easily
# Had many errors due to this method's bug!!!!
def gen_starting_point(input, is_origin=False, is_left=True):
    ...
    result['gt_image'] = img[None]  # (1,1,16,16)  NCHW
