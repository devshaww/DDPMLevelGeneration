...
def left_half_cropping_bbox(img_shape=(16, 16)):
    h, w = img_shape
    top, left, height, width = 0, 0, h, w // 2
    return top, left, height, width


def right_half_cropping_bbox(img_shape=(16, 16)):
    h, w = img_shape
    top, left, height, width = 0, w // 2, h, w // 2
    return top, left, height, width


def random_cropping_bbox(img_shape=(16,16), mask_mode='onedirection'):
    h, w = img_shape
    if mask_mode == 'onedirection':
        _type = np.random.randint(0, 4)
        if _type == 0:
            top, left, height, width = 0, 0, h, w//2     # left half
        elif _type == 1:
            top, left, height, width = 0, 0, h//2, w     # top half
        elif _type == 2:
            top, left, height, width = h//2, 0, h//2, w  # bottom half
        elif _type == 3:
            top, left, height, width = 0, w//2, h, w//2  # right half
    else:
        target_area = (h*w)//2
...
def right_half_cropping_box(img_shape):
    h, w = img_shape[:2]
    return [0, w//2, h, w//2]


def left_half_cropping_box(img_shape):
    h, w = img_shape[:2]
    return [0, 0, h, w//2]
