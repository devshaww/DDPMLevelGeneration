...
class Network(BaseNetwork):
    ...
    def set_new_noise_schedule(self, device=torch.device('cpu'), phase='train'):
    ...
    @torch.no_grad()
    def restoration(self, y_cond, y_t=None, y_0=None, mask=None, sample_num=8, label=None):
        # y_0: ground truth
        b, *_ = y_cond.shape    # num of sampling images, normally it's just 1
        ...
        for i in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps, position=0, leave=True):
            ...
            if i % sample_inter == 0:
                # visualize process: concatenate noisy one and generated one every 250 steps
