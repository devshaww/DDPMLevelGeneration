...
class VisualWriter():
    ...
    def save_images(self, results, tileset, spritesheet, filename=None):
        ...
        # os.makedirs(result_path, exist_ok=True)
        result_path = os.path.join(self.result_dir, str(self.epoch))  # result_dir: experiments/train_uncropping_scenes_timestamp/results/val/1
        os.makedirs(result_path, exist_ok=True)
        ...
        try:
            ...
            #txt = Util.tensor2txt(results['result'][2])  # WARNING: only works when output result is at index 2
            outputs = Util.postprocess(results['result'], tileset, spritesheet)
